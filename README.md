...
对于单层室内消火栓管网，不需要对齐xy坐标，但多层室内消火栓管网，必须对齐xy坐标，这通过在cad中放置对齐点图块来实现，程序读取时，根据对齐点图块的坐标来对齐不同楼层的xy坐标。
...
注：消火栓图块由用户自行在CAD中放置，软件不提供；其它图块在cadblock目录下，包括：供水点（supply_node.dwg）、用水点（demand_node.dwg）、阀门（valve.dwg）和对齐点（floorbase.dwg）。

供水点块用于放在管网的引入管起端，用水点块放在用户需要计算的出水点，阀门图块插入在管道上。这三个都可以在程序中添加，不需要在cad中提前放置。
