...
```
pipe_loss_calculator/
├── main.py                    # 主程序入口
├── config.json                # 用户配置（方案、全局设置）
├── materials.json             # 用户管材数据
├── default_materials.json     # 默认管材数据（只读，用于恢复）
├── k_dn_map.json              # 喷头K值-管径映射表
├── k_dn_map_default.json      # 默认K值-管径映射表（只读，用于恢复）
├── floor_pipes_colors.json    # 楼层管道颜色表
├── default_floor_pipes_colors.json  # 默认楼层管道颜色表（只读，用于恢复）
├── cad_cache.json             # CAD解析缓存（自动生成）
├── requirements.txt           # Python依赖列表
├── cadblock/                  # CAD图块文件（含 Floorbase.dwg）
│   └── Floorbase.dwg
├── config/                    # 配置管理模块
├── core/                      # 核心计算模块（INP生成、WNTR计算、结果解析、压力驱动求解）
├── gui/                       # GUI页面模块（设置、管道、节点、供水用水点、阀门、预览、计算）
├── epanet2.2/                 # 保留目录，不再使用（引擎已切换为WNTR）
├── projects/                  # 项目数据目录（自动生成）
│   └── project_xxx/
│       ├── project_meta.json  # 项目元数据
│       ├── network.inp        # EPANET输入文件
│       ├── report.rpt         # 计算报告
│       └── results.json       # 计算结果
├── projects_temp/             # 模式B/C迭代临时文件（自动生成）
├── exports/                   # 导出的项目模型（.zip）
├── examples/                  # 示例CAD文件
└── img/                       # 示例截图
```
...
8、程序左上角有个叫文件的菜单项，里面有导入和导出功能，可以导出和导入数据模型。

9、软件目录下的exports目录下是四个管网模型样例，对应四个示例文件，可以用导入功能读取。
...
重要提示：选此管检修时，不要选有阀门的管道，程序因为判断逻辑的原因，选有阀门的管道时，会错误扩大检修管道的范围。
...
- **CAD文件路径**：点击“浏览”按钮，选择您的CAD图纸文件（支持 `.dwg` 格式）。选择后，路径会显示在输入框中。
