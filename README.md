...
对于dwg文件，尽量减少与需要识别到管网无关的图元，极端的话，可以删除包括建筑底图在内的所有无关图元，这样可以加快读取速度。当然，dwg只要读取一次，就可以导出数据模型，以后直接导入数据模型就行。导入导出数据模型非常快，所以，dwg中是否连建筑底图等图元都删除纯粹看个人选择。
...
部分dwg支持dxf导出读取，速度会很快，但部分dwg不支持dxf导出读取，速度会比较慢，有的会需要3分钟以上。对于这种dwg，建议读取完成后先导出数据模型，以后就可以直接导入数据模型了，导入数据模型是最快的。
