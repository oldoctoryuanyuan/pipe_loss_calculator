...
注意，本程序使用的计算模块是wntr，请仔细搜索wntr是什么。其它请根据requirements.txt安装所需运行库。

这个软件主要想要解决的问题包括：

1、一个40L/s的复杂的室内消火栓管网，其中的流量和压力分布究竟如何？底部楼层的横向环网是否必须使用DN150？能否将常规设计为DN150的管道优化为DN100？

2、能否为设计师提供一个环状喷淋管网的免费计算功能？
...
室外消火栓管网计算结果只能是估算结果，不能作为精确结果，因为建筑的不同引入管的水压其实是不一样的，程序假定同一栋建筑的不同引入管的引入点的水压是一样的。室外消火栓也一样，各个室外消火栓的压力也会不同，所以这个管网类型的计算只能是估算结果。

室内消火栓管网，程序可以读取多层或单层室内消火栓管网。并计算管网的流量分配和各管段水损。此管网的计算结果是精确结果。

对于单层室内消火栓管网，不需要对齐点图块。但多层室内消火栓管网，必须放置对齐点图块。

对齐点图块位于软件目录的cadblock目录下，名为floorbase.dwg，用户将每一层管网都用一个矩形框完全框住，并在矩形框左下角点插入对齐点图块。矩形框必须跟对齐点图块位于同一同层，不允许有其它图元使用它们的图层。

每一层的对齐点插入位置（矩形框的左下角点位置）必须在建筑坐标轴上对齐，比如都在A轴交1轴处。对齐点图块是为了将不同楼层的管网在z坐标上对齐，用户插入对齐点时必须重视这一点。

对齐点块有个参数，参数格式是：楼层名_楼面标高，比如；一层_0.00、十层_42.00。当多层共用一个平面时，各层楼面标高用&分隔，比如：三至五层_14.80&18.90&23.00。

喷淋管网，程序可以计算单层最不利作用面积管网，理论上能计算环状管网和货架内喷淋管网，但尚未测试。此管网的计算结果是精确结果。

用户需要在作用面积内的每个喷头处放置用水点图块。用水点图块位于软件目录的cadblock目录下，名为demand_node.dwg。

需要注意的是，程序对三类管网都只支持单个用水点组的计算。

对于这三种管网，管道、立管圆和立管标注都必须有对应的图层，并在软件的设置页面输入图层名称。

喷淋管网必须在计算作用面积内的喷头处放置用水点图块，另两种管网的用水点和三种管网的供水点都可以在程序的管网预览页面中设置。

当然，用户也可以提前在cad中放好用水点图块和供水点图块，供水点图块位于软件目录的cadblock目录下，名为supply_node.dwg。

不支持天正等专业软件格式，推荐用天正转t3导出的dwg文件，其它画图软件画的dwg理论上也能识别，但未测试。

当使用其它软件画cad时，建议注意凡是代表管道的线段相连处，两条线段的端点尽量重合，程序设定了容差10mm（用户可修改），但尽量不要依赖容差。

对于多段线，程序会将至拆分为多个管道；一条线段端点落在另一条线段上时，程序会将相交点理解为三通；两条线段相交（相交点非两条线段的端点）时，程序会将相交点理解为四通。

需要注意，当因为图面识别需求，被上方管道遮挡而打断的管道，程序不会自动将其连接，只会认为是不相连的两段管道。这个没办法，程序没这么智能。


程序设置页面的管理按钮可以修改管材C值，按钮下方可以选择管材。设置页面右方是颜色和管径对照表（此表内容可修改），用户如果愿意，可以在cad中将不同管径的线段颜色按此表设置，程序读取时就能直接获得管径。

如果嫌麻烦，也可以不进行赋予颜色的步骤，在程序的管网预览界面中修改管径。程序在读取cad时，会询问不符合颜色管径对照表的线段赋予什么管径。

代表管道的线段可以是line或polyline。立管圆可以是circle或arc，因为天正转t3的图中，管道下翻的圆会被转化为arc，所以程序保留识别圆弧的能力。

但程序设计时没有考虑弧形管道，不能确保能识别，也不能确保程序使用的wntr计算模块能识别，但用户可以尝试。
...
5、图块设置中输入供水块名、用水块名和阀门块名、对齐点块名，以及图块的属性字段名。

注：消火栓图块由用户自行在CAD中放置，软件不提供；其它图块在cadblock目录下，包括：供水块（supply_node.dwg）、用水块名（demand_node.dwg）、阀门块名（valve.dwg）和对齐点块（floorbase.dwg）。

供水点块用于放在管网的起端，用水点块放在用户需要计算的出水点。阀门图块插入在管道上。

如前所述，喷淋管网的喷头必须在cad中放置用水点图块，其它可以在程序的管网预览界面自行添加和删除。

6、计算公式栏中是喷淋和消火栓的计算公式参数，一般保持默认值。喷头K值需要注意选择，也可以自行修改。

最高最低流速自行设置。当计算完成后，发现某些管道流速高于最高流速或低于最低流速，可以在计算校正或优化管径后重新计算。

7、点击“读取文件”按钮，读取cad数据。耐心等待程序读取完成。

8、读取有两个模式，分别是DXF模式和COM模式，缺省选择DXF模式，这个模式速度快。DXF模式下实体属性读取与COM模式基本一致，如发现异常可去掉DXF读取复选框，使用COM模式读取，这个模式速度慢，但能读取完整。

9、顶部有个文件菜单项，里面有导入和导出功能，可以导出和导入cad模型。当你读取了一个cad，并且进行了调整和计算后，可以选择导出项目，以后直接导入就行，不需要重新读取cad，再调整和计算。

10、软件目录下的exports目录下是四个管网模型样例，对应四个示例文件。

### 0.3 读取cad后的操作
1、管道页面、节点页面和阀门页面是程序读取的管道、节点和阀门列表，供数据查看用。

2、供水点和用水点页面很重要，如果没有在cad中放置供水点图块和用水点图块，可以先去管网预览页面放置。

3、对于单层管网，管网预览页面只有单层预览标签，但多层管网，除了每层的平面管网预览标签外，还有个整体管网预览标签，相当于轴测系统图。
...
9、在管网预览页面，选择一端为自由节点（指只连一根管道的点）的管道，点右键，可以放置或删除供水点或用水点。可以选择多个管道来放置（鼠标在空白处点右键，点选择集命令）。

10、右键菜单放置用水点后，每个用水点都是一个独立组，需要保持选择它们的状态（实际上就是选择所在管道），选右键菜单的用水点编组，选择或输入一个组名，将这些用水点编成一组。

11、供水点和用水点放置和编组完毕（供水点不需要编组）后，到供水点和用水点页面，根据你想要计算的方式，输入数据。

12、室外消火栓管网的计算是必须勾选要计算的用水点组，输入其总流量和供水点压力，然后去计算页面计算。

13、室内消火栓管网和喷淋管网，必须勾选要计算的用水点组，然后又两种计算模式，一种是只输入供水点压力，勾选的用水点组的总流量和最低水压保持为零，去计算页面点开始计算，这是正向计算。另一种是供水点压力和用水点组总流量保持为零，只输入用水点组的最低水压，去计算页面点开始计算，这是反向计算。这个最低水压是指用水点组中计算后水压最低的那个点的水压，比如计算喷淋管网时，输入10，就代表满足最不利喷头工作压力10m时，管网的流量和压力分配。计算室内管网时，输入35，就代表最不利消火栓栓口工作压力35m时，管网的流量和压力分配。

14、计算完成后，计算页面可以看到节点结果和管道结果表，还有路径表，路径表值供水点到用水点的某条路径，可以看到路径上的总水损。

15、计算页面中的结果不算直观，可以切换回管网预览页面查看，显示选项中可以查看管道流量、水损、流向和节点压力。

16、预览页面可以添加删除阀门。

17、对于不检修的环状消火栓管网，计算结果的意义不到，我们需要考虑的是检修情况。阀门设置完毕后，可以在某段管道上右键选此管检修，程序会自动关闭涉及到的阀门，踢除不能过水的管道。然后再计算，就可以知道某段管道检修时，整个管网的水压和流量分布了。计算完后，在检修管道群任一管道右键选不再检修，恢复管道状态。再去选别的管道检修和计算。

18、我们可以通过检修状态的计算来了解我们需要多少供水压力和管网需要的管径。目前程序没有能力自动设置管网各部分进入检修状态，自行计算，这个可能需要更复杂的程序设计。
...
- **DXF读取**复选框：默认勾选。勾选后，程序将通过导出临时DXF文件的方式快速读取CAD实体数据，速度更快。若遇到某些图元（如自定义实体）读取不全的情况，可取消勾选，程序将使用传统的COM接口逐一遍历模型空间，速度较慢但兼容性更好。
...
这里设置供水点、用水点、阀门、消火栓、对齐点在CAD中的图块名称和属性字段。

- **供水点图块**：输入表示供水点的图块名称（默认 `supply_node`），属性字段（默认 `GroupID`）。
- **用水点图块**：输入表示用水点的图块名称（默认 `demand_node`），属性字段（默认 `GroupID`）。
- **阀门图块**：输入表示阀门的图块名称（默认 `valve`），属性字段（默认 `Status`）。程序不再读取属性值，所有阀门初始状态为OPEN。
- **消火栓图块**：输入表示消火栓的图块名称（默认 `hydrant`），**无属性字段**。程序会将此图块识别为消火栓。
- **对齐点图块**：输入表示对齐点的图块名称（默认 `Floorbase`），属性字段（默认 `Elevation`）。对齐点用于对齐各层管网。
...
> **提示**：模式B/C的结果表格中会自动过滤掉虚拟节点（如`RESERVOIR`、`D_xxx`）和虚拟管道（如`VP_xxx`），只显示实际管网元素。
...
在执行管径调整、阀门操作、消火栓添加/删除、用水点组变更、管道有效性修改、删除等操作后，您可以使用 **Ctrl+Z** 撤销最近的操作（最多20步）。撤销后，相关数据会回滚，并刷新所有页面。重做功能目前未实现。
...
- **罗盘**：点击不同扇区切换12个固定观察方向（30°扇区，红色高亮当前方向）。
...
A: 目前仅支持通过罗盘选择12个固定方向和修改高度角，无法自由旋转。

---

如果您在使用过程中遇到其他问题，可以查看程序目录下的 `app.log` 日志文件，或联系软件开发者。祝您使用愉快！
